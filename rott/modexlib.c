...
signed short mx = 0, my = 0;
word mb = 0;

static unsigned char screenpixels[320 * 200];
...
static void atari_input_shutdown(void);

#ifndef ATARI_ENABLE_KBDINT
...
int ATARI_RenderAllowed(void) { return 1; }
void ATARI_EndRenderFrame(void) {}
void ATARI_ForceRender(void) {}
#endif
...
static void atari_draw_char(int x, int y, char ch, unsigned char color)
{
   ...
   for (ry = 0; ry < 5; ++ry)
   {
      unsigned char bits = font[idx][ry];
      int dst = (y + ry) * iGLOBAL_SCREENWIDTH + x;
      if (bits & 0x4)
         screenpixels[dst] = color;
      if (bits & 0x2)
         screenpixels[dst + 1] = color;
      if (bits & 0x1)
         screenpixels[dst + 2] = color;
   }
}

static void atari_draw_number(int x, int y, int value, unsigned char color)
{
   char buf[16];
   int len = 0;
   int i;
   ...
   for (i = 0; i < len; ++i)
      atari_draw_char(x + i * 4, y, buf[i], color);
}

static void atari_draw_text(int x, int y, const char *text, unsigned char color)
{
   while (*text)
   {
      atari_draw_char(x, y, *text++, color);
      x += 4;
   }
}

static void atari_fill_rect(int x, int y, int w, int h, unsigned char color)
{
   int row;
   if (w <= 0 || h <= 0)
      return;
   ...
   if (x >= iGLOBAL_SCREENWIDTH || y >= iGLOBAL_SCREENHEIGHT)
      return;
   if (x + w > iGLOBAL_SCREENWIDTH)
      w = iGLOBAL_SCREENWIDTH - x;
   if (y + h > iGLOBAL_SCREENHEIGHT)
      h = iGLOBAL_SCREENHEIGHT - y;
   for (row = 0; row < h; ++row)
...
static void atari_draw_fps_overlay(void)
{
   ...
   {
      if (elapsed > 0)
         fps_value = (fps_frames * VBLCOUNTER + (elapsed >> 1)) / elapsed;
      else
         fps_value = 0;
      fps_frames = 0;
      fps_last_tic = now;
   ...
   y = iGLOBAL_SCREENHEIGHT - 7;
   if (x < 0)
      x = 0;
   if (y < 0)
      y = 0;

   atari_fill_rect(x, y, text_w + 2, 7, atari_overlay_bg_color);
   atari_draw_text(x + 1, y + 1, buf, atari_overlay_text_color);
}
#endif
...
{
   atari_dbg("ATARI: GraphicsMode start\r\n");
   atari_dbg("ATARI: GraphicsMode fast_mode\r\n");
   atari_set_fast_mode();
   atari_dbg("ATARI: GraphicsMode Getrez\r\n");
   atari_prev_rez = Getrez();
   ...
   atari_dbg("ATARI: GraphicsMode c2p_init\r\n");
   atari_c2p_init();
...
void I_SetPalette(byte *palette)
{
   ...
   int best_luma = -1;
   int worst_luma = 1 << 30;
   unsigned char best_idx = 255;
   unsigned char worst_idx = 0;
...
#define ATARI_C2P_VIEW_ZOOM 1
   ...
   {
      int screen_y = screenofs / iGLOBAL_SCREENWIDTH;
      int screen_x = screenofs - (screen_y * iGLOBAL_SCREENWIDTH);
      ...
      if (zoom > 1)
      {
         center_x = view_x + (view_w >> 1);
         center_y = view_y + (view_h >> 1);
      }
   }
#endif
#endif
...
void VL_SetVGAPlaneMode(void)
{
   ...
   atari_dbg("ATARI: VL_SetVGAPlaneMode start\r\n");
   GraphicsMode();
   atari_dbg("ATARI: VL_SetVGAPlaneMode after GraphicsMode\r\n");
   ...
   atari_dbg("ATARI: VL_SetVGAPlaneMode before XFlipPage\r\n");
   XFlipPage();
   atari_dbg("ATARI: VL_SetVGAPlaneMode done\r\n");
}
...
{
   if (buffer != page1start)
      memcpy(page1start, buffer, screensize);
   if (page2start != page1start && buffer != page2start)
      memcpy(page2start, buffer, screensize);
   if (page3start != page1start && page3start != page2start && buffer != page3start)
...
static void *atari_old_interrupt_handler = NULL;

#define ACIA_RX_OVERRUN (1 << 5)
#define ACIA_RX_DATA (1 << 0)
static volatile unsigned char *atari_pAciaCtrl = (void *)0xfffc00;
static volatile unsigned char *atari_pAciaData = (void *)0xfffc02;
static volatile unsigned char *atari_pMfpIsrb = (void *)0xfffa11;

static unsigned char atari_input_buffer[256];
static unsigned char *atari_next_read = atari_input_buffer;
static unsigned char *atari_next_write = atari_input_buffer;
static int atari_pending_keyup = -1;
static int atari_pending_keyup_delay = 0;

static int atari_input_buffer_full(void)
{
   unsigned char *p = atari_next_write + 1;
   if (p == atari_input_buffer + sizeof(atari_input_buffer))
      p = atari_input_buffer;
   return p == atari_next_read;
}

static int atari_input_buffer_empty(void)
{
   return atari_next_read == atari_next_write;
}

__attribute__((interrupt)) static void atari_keyboard_interrupt(void)
{
   unsigned char aciaCtrl = *atari_pAciaCtrl;
   ...
   else
   {
      ...
      {
         if (atari_input_buffer_full())
            break;
         *atari_next_write++ = *atari_pAciaData;
         if (atari_next_write == atari_input_buffer + sizeof(atari_input_buffer))
            atari_next_write = atari_input_buffer;
         aciaCtrl = *atari_pAciaCtrl;
      }
   }
   *atari_pMfpIsrb &= ~(1 << 6);
}

static int atari_can_read_bytes(size_t num)
{
   if (num > sizeof(atari_input_buffer))
      return 0;
   unsigned char *p = atari_next_read;
   while (p != atari_next_write)
   {
      if (--num == 0)
         return 1;
      if (++p == atari_input_buffer + sizeof(atari_input_buffer))
...
static int atari_read_byte(void)
{
   int data = *atari_next_read++;
   if (atari_next_read == atari_input_buffer + sizeof(atari_input_buffer))
      atari_next_read = atari_input_buffer;
   return data;
}

static int atari_peek_byte(void)
{
   return *atari_next_read;
}

static int atari_translate_scancode(int code)
{
   if (code == 0x61)
      return sc_F12; // Undo
   if (code == 0x62)
...
static void atari_enqueue_key(unsigned char data)
{
   unsigned char k;
   int next_tail;
   ...
   if (data & 0x80)
   {
      k = (unsigned char)atari_translate_scancode(data & 0x7f) | 0x80;
      Keystate[k & 0x7f] = 0;
   }
   else
   {
   ...
   next_tail = (Keytail + 1) & (KEYQMAX - 1);
   if (next_tail == Keyhead)
...
static void atari_input_init(void)
{
   ...
   if (!ATARI_ENABLE_KBDINT)
   {
      atari_dbg("ATARI: input_init disabled\r\n");
      return;
   }
   atari_next_read = atari_input_buffer;
   atari_next_write = atari_input_buffer;
   if (!atari_old_interrupt_handler)
   {
      long old = Super(0);
      atari_old_interrupt_handler = *(void **)0x118;
      Super(old);
      atari_super_set_vector((void **)0x118, atari_keyboard_interrupt);
   }
   atari_dbg("ATARI: input_init done\r\n");
}
...
static void atari_input_shutdown(void)
{
   ...
   if (atari_old_interrupt_handler)
   {
      atari_super_set_vector((void **)0x118, atari_old_interrupt_handler);
      atari_old_interrupt_handler = NULL;
   }
}

void doEvents(void)
{
   if (atari_pending_keyup >= 0)
   {
      if (atari_pending_keyup_delay > 0)
      {
         atari_pending_keyup_delay--;
      }
      else if (atari_input_buffer_empty())
      {
         atari_enqueue_key((unsigned char)(atari_pending_keyup | 0x80));
         atari_pending_keyup = -1;
      }
   }

   while (!atari_input_buffer_empty())
   {
      unsigned char data = (unsigned char)atari_peek_byte();

      if (data >= 0xf8 && data <= 0xfb)
      {
         if (!atari_can_read_bytes(3))
            break;
         atari_read_byte();
         {
            signed char dx = (signed char)atari_read_byte();
            signed char dy = (signed char)atari_read_byte();
            word buttons = 0;
            if (data & 0x01)
               buttons |= 1; /* left */
            if (data & 0x02)
      ...
      else if (data == 0xfd)
      {
         ...
         atari_read_byte();
         {
            unsigned char joy0 = (unsigned char)atari_read_byte();
            unsigned char joy1 = (unsigned char)atari_read_byte();
            (void)joy1;
            atari_joy_buttons = (joy0 & 0x10) ? 1 : 0;
            Joy_x = 32768;
            Joy_y = 32768;
            if (joy0 & 0x04)
               Joy_x = 0; /* left */
   ...
   if (atari_input_buffer_empty())
   {
      ...
      {
         long v = Cconin();
         unsigned char scan = (unsigned char)((v >> 16) & 0xff);
         unsigned char ascii = (unsigned char)(v & 0xff);
         if (scan == 0)
         {
            if (ascii == 0x1b)
               scan = sc_Escape;
         ...
         if (scan != 0)
         {
            atari_enqueue_key(scan);
            atari_pending_keyup = scan;
            atari_pending_keyup_delay = 8;
         }
      }
   }
}
